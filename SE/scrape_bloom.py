...
ALLOWED_DOMAINS = ["https://www.npr.com"]
pattern = r"^https://www\.npr\.org/\d{4}/\d{2}/\d{2}/.*$"
...
def build_web_graph(start_urls, max_depth, save_interval=10):
    ...
    while queue:
        ...
        if processed_count % save_interval == 0:
            os.system("rm webgraph_npr.json")
            print(f"Saving progress at {processed_count} URLs processed...")
            save_graph(graph, "webgraph_npr.json")

    return graph


# Function to save the graph to JSON
def save_graph(graph, filename="webgraph_npr.json"):
    data = {
